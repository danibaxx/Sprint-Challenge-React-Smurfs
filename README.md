...
- [ ] Explain the differences between `client-side routing` and `server-side routing`.
  ...
  - Client-side routing: Handeled internarlly by JavaScript that is loaded on the page.
  - Server-side routing: Requesting information from a server by clicking on a link or typing in a web address.
...
- [ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.
  ...
  - Create can be mapped by Post, Read can be mapped by Get, Updated can be mapped by Put, and Delete can be mapped by Delete.

- [ ] Mention three tools we can use to make AJAX requests.

  - Rendering data with state and props, using HTTP libraries like axios, and Fetching the data with componentDidMount() call.
